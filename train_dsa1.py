...
# === Model 2: DSA_TransUNet with Sparse Attention ===
...
def train_model(model_class, model_name):
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch+1) % 10 == 0 or (epoch+1) == EPOCHS:
            path = os.path.join(SAVE_PATH, f"{model_name}_epoch_{epoch+1}.pth")
            torch.save(model.state_dict(), path)
            print(f"Saved model: {path}")
    np.save(f"stepwise_losses_{model_name}.npy", np.array(stepwise_losses))
    print(f"Saved stepwise loss as stepwise_losses_{model_name}.npy")

# === Choose Model and Train ===
if __name__ == "__main__":
    # Use one of the following lines based on which model you want to train:
    
    # Train SimpleTransUNet:
    # train_model(model_class=SimpleTransUNet, model_name="transunet")

    # Train DSA_TransUNet:
    train_model(model_class=DSA_TransUNet, model_name="dsa_transunet")
